...
func Task1(data string, testRow int) (result framework.Result[int]) {
	sensors := parse(data)
...
func parse(data string) []model.Sensor {
	return lo.Map(framework.Lines(data), func(line string, index int) model.Sensor {
		...
		return model.NewSensor(geometry.Point{X: x, Y: y}, geometry.Point{X: beaconX, Y: beaconY})
	})
}
